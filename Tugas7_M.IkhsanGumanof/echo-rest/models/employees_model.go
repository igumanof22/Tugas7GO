...
import (
	...
	cm "Tugas7_M.IkhsanGumanof/echo-rest/common"
...
func UpdateEmployee(e echo.Context) (res Response, err error) {
	...
	sqlStatement := `UPDATE employees SET Title = ?, Address = ? WHERE   FirstName= '` + req.FirstName + `'`
	...
	result, err := stmt.Exec(req.Title, req.Address)
